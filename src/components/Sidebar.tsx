...
export function Sidebar({ nav, active, onNavigate }: SidebarProps) {
  return (
    <aside
      style={{
        background: 'white',
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 10.5, color: '#7A6E66', marginTop: 3 }}>Panel del comerciante</div>
          </div>
        </div>
      </div>
      ...
      <nav style={{ padding: '8px 10px 0', display: 'flex', flexDirection: 'column', gap: 2 }}>
        {nav.map((sec) => (
          <button key={sec.id} onClick={() => onNavigate(sec.id)} style={navItemStyle(sec.id === active)}>
            <span style={{ fontSize: 15, width: 20, textAlign: 'center', lineHeight: 1 }}>{sec.icon}</span>
            <span>{sec.name}</span>
          </button>
        ))}
      </nav>
